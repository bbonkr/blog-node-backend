import { EXCERPT_LENGTH, stripHtml, getExcerpt } from '../post.helper';
...
test('stripHtml - nested', () => {
    const html = '<div><h1>Hello world</h1><p>and hello world again</p></div>';

    const result = stripHtml(html);

    expect(result).toBe('Hello worldand hello world again');
});

test('stripHtml - invalid html', () => {
    const html = '<div><h1>Hello world<p>and hello world again</p></div>';
...
test('getExcerpt - long text', () => {
    ...
    const result = getExcerpt(longText, EXCERPT_LENGTH);

    expect(result.length).toBe(EXCERPT_LENGTH);
});

test('getExcerpt - short text', () => {
    const shortText = 'hello world';
    const result = getExcerpt(shortText, EXCERPT_LENGTH);
    expect(result.length).toBe(shortText.length);
});

test('getExcerpt - empty', () => {
    const shortText = '';
    const result = getExcerpt(shortText, EXCERPT_LENGTH);
